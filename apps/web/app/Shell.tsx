...
export default function Shell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    setUserName(localStorage.getItem("zapye_user_name") || "Admin");
    setUserEmail(localStorage.getItem("zapye_user_email") || "");
  }, []);

  if (bare(pathname)) return <>{children}</>;

  const initials = userName.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase() || "A";
      ...
      <aside className="app-sidebar hidden min-h-screen w-[246px] shrink-0 border-r px-6 py-9 md:flex md:flex-col" style={{ borderColor: "var(--border)" }}>
        ...
        <div className="mt-[170px] space-y-[70px]">
          ...
          <div className="sidebar-admin-card flex items-center gap-3 p-4">
            <div className="grid h-10 w-10 place-items-center overflow-hidden rounded-full bg-[#f4d6c5] text-sm font-black">{initials}</div>
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-bold">{userName}</div>
              <div className="truncate text-xs muted-ink">{userEmail}</div>
            </div>
